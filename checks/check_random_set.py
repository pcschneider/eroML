...
gi0 = np.where(ff["category"] == 1)[0]
plt.scatter(ff["bp_rp"][gi0], np.log10(ff["FxFg"][gi0]), color='0.7', marker='.')

gi1 = np.where(ff["category"] == 0)[0]
plt.scatter(ff["bp_rp"][gi1], np.log10(ff["FxFg"][gi1]), color='0.7', marker='.')

#gq  = ff["Gaia_Quality"] == "True"
#ff["Gaia_Quality"] = gq
...
print(len(gi2))
plt.scatter(ff["bp_rp"][gi2], np.log10(ff["FxFg"][gi2]), label="stars/iso", s=15, facecolor="none", edgecolor='b')
